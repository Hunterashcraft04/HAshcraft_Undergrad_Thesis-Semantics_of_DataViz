...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function run_start_time = wait_for_trigger_kbqueue_all_dvc(delay_milliseconds, esc_key_latency)
  arguments
  ...
  % Jin added:
  for i = 1:numel(kb_inds)
      KbQueueCreate(kb_inds(i),keyList);
      KbQueueStart(kb_inds(i));
      KbQueueFlush(kb_inds(i));
  end
  ...
  % Jin added
  fprintf('WAITING FOR TRIGGER on device = %s.\nTo ESC, hold down ESC key for at least %.2f seconds.\n', mat2str(kb_inds), esc_key_latency);
  ...
  % Jin added
  % while ~KbCheck(-1) && isempty(evt)
  %     perDevWait = esc_key_latency/max(1,numel(kb_inds));
  %     for i = 1:numel(kb_inds)
  %         e = KbEventGet(kb_inds(i),perDevWait);
  %         if ~isempty(e)
  %             evt = e; break;
  %         end
  %     end
  % end
    ...
    while ~KbCheck(-1) && isempty(evt)
        ...
        for i = 1:numel(kb_inds)
            e = KbEventGet(kb_inds(i), perDevWait);
            if ~isempty(e)
                fprintf('Device %d saw keycode %d (%s)\n', kb_inds(i), e.Keycode, KbName(e.Keycode));
                evt = e;
                break;
            end
        end
  ...
  for i=1:numel(kb_inds)
      KbEventFlush(kb_inds(i));
      KbQueueRelease(kb_inds(i));
  end
